...
- Long assistant Markdown without math is also split into bounded top-level Compose rows. This keeps later native tables reachable by physical horizontal drag instead of leaving the full message as one oversized `LazyColumn` item.
