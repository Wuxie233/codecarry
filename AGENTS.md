...
- Long assistant Markdown without math is split into typed top-level Compose rows. Tables and fenced code own their rows; root lists, blockquotes, raw HTML, and indented code remain atomic; prose may coalesce within the size budget. There is no whole-message/max-chunks fallback.
- Compose rendering dispatches typed Markdown blocks independently. Tables use the measured `MarkdownTableLayout.kt` renderer with 80..280dp content-adaptive columns; only a chunk containing a math placeholder routes through the KaTeX/WebView renderer. Keep prose selection intact: JVM gesture diagnosis proved `SelectionContainer + DisableSelection` does not block table dragging.
