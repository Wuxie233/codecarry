...
- Long assistant Markdown without math is also split into bounded top-level Compose rows. Root lists, blockquotes, raw HTML, and indented code stay atomic inside that plan; they must not trigger a whole-message fallback that leaves later native tables inside one oversized `LazyColumn` item.
