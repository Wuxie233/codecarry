...
class SessionStatusSnapshotTest {
    ...
    private class StatusSnapshotTransport(
        ...
        override suspend fun listRooms(
            directory: String?,
            rootsOnly: Boolean,
        ) = emptyList<dev.wuxie233.codecarry.domain.transport.TransportRoom>()

        override suspend fun listRoomScopes() = emptyList<dev.wuxie233.codecarry.domain.transport.TransportRoomScope>()

        override fun openEventStream(directory: String?) = kotlinx.coroutines.flow.emptyFlow<dev.wuxie233.codecarry.domain.transport.TransportEvent>()
        ...
        override suspend fun sendMessage(
            roomId: String,
            parts: List<dev.wuxie233.codecarry.domain.transport.TransportMessagePart>,
            model: dev.wuxie233.codecarry.domain.transport.TransportModelSelection?,
            agent: String?,
            variant: String?,
            directory: String?,
        ) = Unit
